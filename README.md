...
- Some actions in R are very time consuming. For example if you have generated a list of 150000 dataframes of ngrams, it might take 41 minutes to rbind all the data frames in the list with `fullset <- do.call(rbind, gramset)` due to memory reallocation, but only take 38 seconds to write them out appending to a single file that can be read back in moments `lapply(gramset, function(x){write.table(x, file="outy.txt", append=TRUE, row.names=FALSE, col.names=FALSE)})` 
